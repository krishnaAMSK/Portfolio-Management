...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      let response = await axios.post("http://localhost:4000/login", credentials);
  ...
  const handleRegister = async (e) => {
    e.preventDefault();
    try {
      let reg_credentials = {email, password, username, contact}; 
      console.log(username);
      let registerRes = await axios.post("http://localhost:4000/register", reg_credentials);
